...
class TestCLI_CaptureSTDOUTERR(unittest.TestCase):
    ...
    def test_earth_glod_missing_datetime(self):
        cli = get_cli()
        errcode = cli.handle_input(
            *get_opts("-e 80,80,2 -o nc,./test_files/cli/cliglod.test.nc")
        )
        self.assertEqual(errcode, 1)
    ...
    def test_timeseries_fake(self):
        ...
        errcode = cli.handle_input(
            *get_opts(
                "-e 80,80,2 -t fakefilefakefile.csv -o nc,./test_files/cli/cliglod.test.nc"
            )
        )
        self.assertEqual(errcode, 1)
    ...
    def test_timeseries_wrong(self):
        ...
        errcode = cli.handle_input(
            *get_opts(
                "-e 80,80,2 -t test_files/csv/export_srf_1.csv -o nc,./test_files/cli/cliglod.test.nc"
            )
        )
        self.assertEqual(errcode, 1)
...
class TestCLI(unittest.TestCase):
    ...
    def test_earth_glod_ok_select_coeff(self):
        ...
        errcode = cli.handle_input(
            *get_opts(
                "-e 80,80,2,2010-10-01T02:02:02 -o nc,./test_files/cli/cliglod.test.nc -C 20231120_v2"
            )
        )
        self.assertEqual(errcode, 0)
...
"True", "show_cimel_points": "True"}\''
    ...
    def test_earth_timeseries_glod_ok(self):
        ...
        errcode = cli.handle_input(
            *get_opts(
                "-e 80,80,2 -t ./test_files/csv/timeseries.csv -o nc,./test_files/cli/cliglod.test.nc"
            )
        )
        self.assertEqual(errcode, 0)
