...
def interpolated_moon_disk_reflectance(
    ...
    if wavelength_nm < wvlens[0]:
        ...
        return interpolated_moon_disk_reflectance(wvlens[0], moon_data, coeffs)
    if wavelength_nm > wvlens[-1]:
        # The extrapolation done is "nearest"
        return interpolated_moon_disk_reflectance(wvlens[-1], moon_data, coeffs)
    apollo_coeffs = coeffs.get_apollo_coefficients()
    if wavelength_nm in wvlens:
        apollo_i = wvlens.index(wavelength_nm)
        return (
            _moon_disk_reflectance(wavelength_nm, moon_data, coeffs)
    ...
    y_values = []
    y_values.append(
        _moon_disk_reflectance(x_values[0], moon_data, coeffs)
        * apollo_coeffs[left_index]
    )
    y_values.append(
        _moon_disk_reflectance(x_values[1], moon_data, coeffs)
...
def calculate_elref(
    ...
    """
    return interpolated_moon_disk_reflectance(wavelength_nm, moon_data, coefficients)
